...
const ProjectsPage = (): JSX.Element => {
  ...
  projects.forEach((project) =>
    project.category.forEach((cat) => {
      if (!category.find((c) => c.value === cat)) category.push({ value: cat, label: cat.toUpperCase() });
    }),
  );
          ...
          <div className="pt-10 ml-4 sm:mx-12 md:mx-16">
              ...
              <div className="mx-auto text-center">
                {category.map((c) => (
                  <button
                    ...
                    className={`${
                      active === c.value ? 'text-blue bg-pink border-transparent' : 'text-white border-pink'
                    } w-32 mx-4
            ...
            <div className="grid md:grid-cols-2 gap-4 place-items-center">
              {projects.map((project: Project) => (
                <ProjectCard project={project} key={project.slug} filter={{ key: 'category', value: active }} />
              ))}
            </div>
          </div>
        </div>
